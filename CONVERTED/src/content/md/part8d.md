...
When adding new persons, we must declare that the cache of Apollo client has to be [updated](/en/part8/react_and_graph_ql#updating-the-cache). The cache can be updated by using the option _refetchQueries_ on the mutation to force <em>ALL_PERSONS</em> query to be rerun.
